...
int ObBootstrap::create_all_partitions()
{
  ...
  std::vector<std::thread> create_ths;
  if (OB_FAIL(check_inner_stat())) {
    LOG_WARN("check_inner_stat failed", K(ret));
  } else {
      ...
      create_ths.emplace_back(
        [&, i, thread_pos](){
          ...
          ObTableCreator table_creator(OB_SYS_TENANT_ID,
                                 SCN::base_scn(),
                                 trans);
          lib::set_thread_name("CreateSySPart");
          if (OB_FAIL(trans.start(&sql_proxy, OB_SYS_TENANT_ID))) {
            LOG_WARN("fail to start trans", KR(ret));
          } else if (OB_FAIL(table_creator.init(false/*need_tablet_cnt_check*/))) {
            LOG_WARN("fail to init tablet creator", KR(ret));
          } else if (OB_FAIL(prepare_create_partition(
    ...
    for (auto &re:th_ret) {
      if(OB_FAIL(re)) {
        LOG_WARN("create partition failed", K(re));
        break;
      }
    }
                                   ...
                                   ObIArray<ObTableSchema> &table_schemas)
{
  ...
  if (table_schemas.count() <= 0) {
    ...
    HEAP_VAR(ObTableSchema, core_table) {
      ...
      } else if (OB_FAIL(batch_create_schema(ddl_service, tmp_tables, 0, 1, false))) {
    ...
    LOG_INFO("[BOOTSTRAP] Create virtual table end");
    // 接下来，单独创建那些需要依赖的表
    begin = 0;
    end = 0;
    thread_pos = 0;
    threads.clear();
    results.clear();
    LOG_INFO("[BOOTSTRAP] Create dep table start");
    ...
    LOG_INFO("[BOOTSTRAP] Create dep table end", K(ret));
  }
