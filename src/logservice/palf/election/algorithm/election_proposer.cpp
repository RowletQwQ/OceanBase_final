...
{
  static int64_t ELECT_GUARD_TIME = 25_ms;
  ...
  {
    ELECT_TIME_GUARD(ELECT_GUARD_TIME);
...
int ElectionProposer::register_renew_lease_task_()
{
  ...
  if (CLICK_FAIL(p_election_->timer_->schedule_task_repeat(renew_lease_task_handle_,
                                                           std::min(int64_t(ELECT_GUARD_TIME), CALCULATE_RENEW_LEASE_INTERVAL()),
                                                           [this]() {
    int ret = OB_SUCCESS;
