...
if __name__ == "__main__":
    ...
    parser.add_argument("--perf-path", dest='perf_path', type=str, help="the path of perf data")
    ...
    parser.add_argument('--gdb-port', dest='gdb_port', type=str, default='1234', help='gdbserver port')
    ...
    deploy_begin = datetime.datetime.now()
    _logger.info("deploy begin")
    ...
    try:
        ...
        if len(server_status) != 1 or server_status[0]['status'] != 'ACTIVE':
            ...
            exit(1)
        checkout_end = datetime.datetime.now()
        # ObRootService::check_config_result
        _logger.info('create tenant begin')
        perf_cmd = f"bash start_record.sh"
        process = subprocess.Popen(perf_cmd, shell=True)
